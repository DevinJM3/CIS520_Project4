...
int main()
{
    ...
    if (taskID == 0){ //For the main process
        for(int i = 1; i < numProcesses; ++i){
            ...
            MPI_Send(&start_line, 1, MPI_UINT32_T, i, 0, MPI_COMM_WORLD); //Send the start line to each process.
        }
        ...
        for(int i = 1; i < numProcesses; ++i){ //Loop to print each process's work
            MPI_Recv(biggest_char_single_process, linesPerProcess, MPI_UINT8_T, i, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); //Receive each array
        ...
        uint32_t start_line;
        MPI_Recv(&start_line, 1, MPI_UINT32_T, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE); //Receives the start line
        processLines(start_line, linesPerProcess, biggest_char_single_process); //process the lines starting at the start line
        MPI_Send(biggest_char_single_process, linesPerProcess, MPI_UINT8_T, 0, 0, MPI_COMM_WORLD); //Sends the array back to the main process
    } 
